...
function getPolynom(...args) {
  return (value) => [...args].reverse().reduce((a, e, i) => a + e * (value ** i), 0);
}
...
function logger(func, logFunc) {
  return (...args) => {
    const header = `${func.name}(${args.map((e) => JSON.stringify(e)).join`,`})`;
    logFunc(`${header} starts`);
